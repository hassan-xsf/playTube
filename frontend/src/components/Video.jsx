...
function Video() {
    ...
    useEffect(() => {
        ; (async () => {
            try {
                ...
                const isSubbed = await axios.get(`/api/v1/sub/hassubbed/${videoId}`)
                if (isSubbed) {
                    setisSubbed(isSubbed.data.data.subbed)
                }
            }
            catch (error) {
    ...
    const handleSubscribe = async () => {
        try {
            await axios.post(`/api/v1/sub/channel/${VideoData.videoOwner.username}`)
            setisSubbed(state => !state)
        }
        catch (error) {
                            ...
                            <div className="flex flex-col lg:flex-row justify-between items-center gap-5 xl:gap-12 pt-4">
                                <div className="flex items-center gap-3.5 pb-1 flex-col lg:flex-row">
                                    <div className="rounded-full size-12 bg-white flex justify-center items-center overflow-hidden">
                                        <Link to={`/c/${VideoData.videoOwner.username}`}>
                                    ...
                                    <div className="flex flex-col">
                                        ...
                                        <span className="font-normal text-black text-sm leading-tight  dark:text-white dark:text-opacity-50">{VideoData.totalSubs} subscribers </span>
                                    </div>
                                </div>
                                <div className="flex justify-between flex-grow gap-1 xsm:gap-5 lg:gap-0">
                                    <div className= {`text-white ${isSubbed ? "bg-black bg-opacity-90 dark:bg-white" : "bg-black bg-opacity-5 dark:bg-gray-100 dark:bg-opacity-10"}  py-3 px-3 xl:px-6 flex justify-start items-center border-black rounded-full`}>
                                        <button onClick = {handleSubscribe} className="flex justify-center items-center hover:scale-105 hover:bg-opacity-10 dark:hover:bg-opacity-30 transition-all">
                                            {
                                                !isSubbed ?
                                                    <>
                                                    ...
                                                    <>
                                                        ...
                                                        <span className="font-semibold text-xs sm:text-md text-white dark:text-black px-3">Subscribed</span>
                                                    </>
                                            }
                                        </button>
